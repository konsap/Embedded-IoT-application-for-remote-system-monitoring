...
void light_thread_create(void);
static void light_thread_func(ULONG thread_input);
static uint8_t light_thread_stack[4096] BSP_PLACE_IN_SECTION_V2(".stack.light_thread") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
TX_EVENT_FLAGS_GROUP g_event_flags_1;
...
void light_thread_create(void)
{
    ...
    UINT err_g_event_flags_1;
    err_g_event_flags_1 = tx_event_flags_create (&g_event_flags_1, (CHAR *) " Event Flags 1");
    if (TX_SUCCESS != err_g_event_flags_1)
    {
        tx_startup_err_callback (&g_event_flags_1, 0);
    }

    UINT err;
    err = tx_thread_create (&light_thread, (CHAR *) "light_thread", light_thread_func, (ULONG) NULL,
                            &light_thread_stack, 4096, 4, 4, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&light_thread, 0);
    }
}
